$(document).ready(function () {
    ...
    var gURL = $app_url + "/files/get-json-data";
    gURL = initURL();
    /*if ($("#parent_id").length > 0) {
        gURL = gURL + "?parent=" + $("#parent_id").val() + "&parent_type=" + $("#parent_type").val();
    }*/
    if ($("#tbldata").length > 0) {
        ...
        cTable = $('#tbldata').DataTable({
            ...
            'columnDefs': [
                ...
                },
                {
                    ...
                    "width": "20%",
                    "className": "text-center",
                    "visible": (($("#storage").length>0 && $("#storage").val()==2) || $role != "ROLE_ADMIN") ? false : true
                }
            ],
    ...
    function filter() {
        ...
        if ($("#parent_id").length > 0) {
            paramArr.push("parent=" + $("#parent_id").val());
            paramArr.push("parent_type=" + $("#parent_type").val());
        }
        var param = paramArr.join("&");
        gURL = "files/get-json-data?" + param;
        cTable.ajax.url(gURL).load();
    }
    ...
    function initURL() {
        ...
        if ($("#filter_storage_type").length > 0) {
            paramArr.push("type=" + $("#filter_storage_type").val());
        }
        if ($("#storage").length > 0 && $("#storage").val() != 0) {
            paramArr.push("storage_type=" + $("#storage").val());
        }
        var param = paramArr.join("&");
        return $app_url + "/files/get-json-data?" + param;
    }
