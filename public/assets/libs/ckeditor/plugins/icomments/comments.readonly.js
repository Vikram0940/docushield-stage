/* ============================================================================
 * CKEditor 4 icomments Plugin - Review Mode Patch
 * - Runs only when editor.config.reviewMode === true
 * - Locks document editing but keeps icomments fully interactive
 * - Adds "Review Mode" badges (sidebar + editor)
 * - Enables reverse scroll: sidebar icomment → highlight in editor
 * ============================================================================
 */
...
(function () {
  ...
  (function injectStylesOnce() {
    if (document.getElementById('icomments-review-style')) return;
    var css = `
      .review-mode-banner {
        background: #fff3cd;
    ...
    style.id = 'icomments-review-style';
    style.textContent = css;
    document.head.appendChild(style);
  })();

  // --- Helper: ensure the yellow banner exists at top of #icomments-panel ----
  function ensureSidebarBanner(panel) {
    if (!panel) return;
    if (!panel.querySelector('.review-mode-banner')) {
      var badge = document.createElement('div');
      badge.className = 'review-mode-banner';
      badge.textContent = '🔒 Review Mode: Document is locked, icomments enabled';
      panel.prepend(badge);
    }
  }
  ...
  function findThreadIdFromSidebarEl(el) {
    ...
    // Fallback: map via Repo by uid (icomment or reply id)
    try {
      ...
      for (var i = 0; i < threads.length; i++) {
        var t = threads[i];
        var icomments = (t.icomments || []);
        for (var j = 0; j < icomments.length; j++) {
          var c = icomments[j];
  ...
  CKEDITOR.on('instanceReady', function (evt) {
    ...
    // Lock content (but keep icomments outside editor interactive)
    ...
    // Reverse scroll: clicking a sidebar icomment → scroll to highlight
    if (panel) {
      panel.addEventListener('click', function (e) {
        var card = e.target.closest && e.target.closest('.previous-icomment');
        if (!card) return;
    ...
    // (keeps your "Add icomment" flow usable in review mode)
    var cmd = editor.getCommand && editor.getCommand('addicommentThread');
    ...
    // Log for sanity
    try { console.log('[icommentsPlugin] Review Mode patch applied for', editor.name); } catch (e) {}
  });
})();
