...
<main class="ds-main flex-grow-1 pb-4">
    <div class="container-fluid">
        <?php
        //if (isset($segments) && in_array("storage", $segments)) {
            if (isset($_GET["parent"])) {
                if ($_GET['parent_type'] == 1) {
                    $project_info = (new \App\Models\Project)->where("id", $_GET['parent'])->first();
                }
                else if ($_GET['parent_type'] == 2) {
                    $folder_info = (new \App\Models\Folders)->find($_GET['parent']);
                    $project_info = (new \App\Models\Project)->where("id", $folder_info->parent_id)->first();
                }                
        ?>
                <div class="inner-subheader mt-2 mb-3">
                    <ul class="breadcrumbs">
                        <li><a href="<?=site_url("dpanel/projects")?>" class='text-dark' style="text-decoration: none;">Projects</a></li>
                        <?php
                        if (isset($project_info)) {
                        ?>
                            <li class="active"><a href="<?=site_url("dpanel/project/detail/".$project_info->case_id)?>" class='text-dark' style="text-decoration: none;"><?=$project_info->name?></a></li>
                        <?php
                        }
                        if (isset($folder_info)) {
                        ?>
                            <li class="active"><a href="<?=site_url("dpanel/project/folder/".$folder_info->id)?>" class='text-dark' style="text-decoration: none;"><?=$folder_info->name?></a></li>
                        <?php
                        }

                        if (isset($segments) && in_array("storage", $segments)) {
                        ?>
                            <li class="active"><?php echo isset($_GET['type']) ? getFileType($_GET["type"])["name"] : "Storage"; ?></li>
                        <?php
                        }
                        else {
                        ?>
                            <li class="active"><?php echo isset($_GET['type']) ? getFileType($_GET["type"])["name"] : "All Files"; ?></li>
                        <?php
                        }
                        ?>
                    </ul>
        ...
        <div class="d-flex align-items-center my-2">
            <h4 class="fw-400 py-3"><?php echo isset($_GET['type']) ? getFileType($_GET["type"])["name"] : "All Files"; ?></h4>
            <?php
            $param = [];
            if (isset($_GET["parent"])) {
            ...
            ?>
                <a class="btn btn-secondary btn-text-secondary fw-300 text-16 ms-auto" href="<?=$addURL?>">
        ...
        if (isset($_GET['parent'])) {
        ?>
            <input type="hidden" name="parent_id" id="parent_id" value="<?php echo isset($_GET['parent']) ? $_GET['parent'] : 0; ?>">
        <?php
        }
        ?>
        <input type="hidden" name="parent_type" id="parent_type" value="<?php echo isset($_GET["parent_type"]) ? $_GET['parent_type'] : 1; ?>">
        <?php
        if (isset($_GET['type'])) {
        ?>
            <input type="hidden" name="filter_storage_type" id="filter_storage_type" value="<?=$_GET['type']?>">
        <?php
        }
        ?>
        <input type="hidden" name="storage" id="storage" value="<?php echo (isset($segments) && in_array("storage", $segments)) ? "2,3" : 0; ?>">
        <!-- Mobile Filters -->
        <!-- Custom Filter Panel -->
        <?php
                        ...
                        <div class="view-_grid p-1">
                            <ul class="ds-icon-menu nav nav-tabs">
                                <li id="deleteRowButton" class="d-none">
                                    <a href="javascript:;" class="btn btn-danger btn-text-danger btn-top-delete me-1" data-url="files/bulk-delete" id="btnBulkDelete" data-message="Are you sure? you want to remove selected files" data-loading="Please wait..">Delete</a>
                                </li>
                                <li>
                                ...
                                <table id="tbldata" class="table align-middle mb-0 table-striped ds-table table-borderless table-project-files w-100">
                                    <thead class="bg-light">
                                        <tr>
                                                ...
                                                <div class="title-with-sort">
                                                    ...
                                                    <div class="dropdown d-inline">
                                                        ...
                                                        <ul class="dropdown-menu sort-dropdown ds-dropdown">
                                                            <li>
                                                                <label class="docushield-checkbox">
                                                                    <input type="checkbox" name="documents" id="chkdocuments" class="checkbox-item" data-col="7" data-order="asc" />
                                                                    Documents
                                                                </label>
                                                            </li>
                                                            <li>
                                                                <label class="docushield-checkbox">
                                                                    <input type="checkbox" name="files" id="chkfiles" class="checkbox-item" data-col="8" data-order="asc" />
                                                                    Files
                                                                </label>
                                                            </li>
                                                            <li>
                                                                <label class="docushield-checkbox">
                                                                    <input type="checkbox" name="images" id="chkimages" class="checkbox-item" data-col="9" data-order="asc" />
                                                                    Images
                                                                </label>
                                                            </li>
                                                        </ul>
                                            ...
                                            <th><?php echo (isset($segments) && in_array("storage", $segments)) ? "File size" : "Activity"; ?></th>
                                            <th>
                                                <div class="title-with-sort">
